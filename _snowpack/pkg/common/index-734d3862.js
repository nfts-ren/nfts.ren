import { c as createCommonjsModule, a as commonjsGlobal } from './_commonjsHelpers-c99fd594.js';
import { B as Buffer$f, b as browser$2, i as isBuffer$1, p as polyfillNode_buffer, s as safeBuffer, a as src, c as src$1 } from './index-62d3c699.js';
import { E as EventEmitter, p as polyfillNode_events } from './polyfill-node:events-0f2be4c8.js';
import { g as global } from './polyfill-node:global-21e5c503.js';
import { p as process, n as nextTick } from './process-e9e98960.js';
import { b as bn, r as require$$0$1 } from './bn-d1fd1360.js';
import { m as minimalisticAssert, i as inherits_browser, h as hash_1 } from './hash-fcae10d4.js';
...
function createHashFunction(hashConstructor) {
    return function (msg) {
        ...
        hash.update(msg);
        return Buffer$f.from(hash.digest());
    };
}
...
function isBuffer(maybeBuf) {
  return Buffer$f.isBuffer(maybeBuf);
}
...
var polyfillNode_string_decoder = /*#__PURE__*/Object.freeze({
    __proto__: null,
...
function validChunk(stream, state, chunk, cb) {
  ...
  } else if (!Buffer$f.isBuffer(chunk) && typeof chunk !== 'string' && chunk !== undefined && !state.objectMode) {
    er = new TypeError('Invalid non-string/buffer chunk');
  }
  if (er) {
    stream.emit('error', er);
...
Writable.prototype.write = function (chunk, encoding, cb) {
  ...
  if (Buffer$f.isBuffer(chunk)) encoding = 'buffer';else if (!encoding) encoding = state.defaultEncoding;
...
function writeOrBuffer(stream, state, chunk, encoding, cb) {
  ...
  if (Buffer$f.isBuffer(chunk)) encoding = 'buffer';
...
const { Transform: Transform$1 } = polyfillNode_stream;

var keccak = (KeccakState) => class Keccak extends Transform$1 {
...
const { Transform: Transform$2 } = polyfillNode_stream;
...
Keccak.prototype.squeeze = function (length) {
  ...
  const output = Buffer$f.alloc(length);
...
var keccak$2 = createCommonjsModule(function (module, exports) {
Object.defineProperty(exports, "__esModule", { value: true });
...
function getRandomBytes(bytes) {
    return new Promise(function (resolve, reject) {
        browser$2(bytes, function (err, resp) {
            if (err) {
                reject(err);
                return;
            }
...
function createPrivateKey() {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    return [4 /*yield*/, random.getRandomBytes(SECP256K1_PRIVATE_KEY_SIZE)];
                case 1:
                    pk = _a.sent();
...
function createPrivateKeySync() {
    while (true) {
        var pk = random.getRandomBytesSync(SECP256K1_PRIVATE_KEY_SIZE);
        if (elliptic$1.privateKeyVerify(pk)) {
            return pk;
        }
    }
...
var ec$2 = new EC$2('secp256k1');
var ecparams$1 = ec$2.curve;

var privateKeyExport = function (privateKey, compressed) {
  var d = new bn(privateKey);
  if (d.ucmp(ecparams$1.n) >= 0) {
    throw new Error('couldn\'t export to DER format');
  }

  var point = ec$2.g.mul(d);
  return toPublicKey(point.getX(), point.getY(), compressed);
};

var privateKeyModInverse = function (privateKey) {
  var bn$1 = new bn(privateKey);
  if (bn$1.ucmp(ecparams$1.n) >= 0 || bn$1.isZero()) {
    throw new Error('private key range is invalid');
  }

  return bn$1.invm(ecparams$1.n).toArrayLike(Buffer$f, 'be', 32);
};

var signatureImport = function (sigObj) {
  ...
  return Buffer$f.concat([r.toArrayLike(Buffer$f, 'be', 32), s.toArrayLike(Buffer$f, 'be', 32)]);
};

var ecdhUnsafe = function (publicKey, privateKey, compressed) {
  var point = ec$2.keyFromPublic(publicKey);
...
var toPublicKey = function toPublicKey(x, y, compressed) {
  ...
  if (compressed) {
    publicKey = Buffer$f.alloc(33);
    publicKey[0] = y.isOdd() ? 0x03 : 0x02;
    x.toArrayLike(Buffer$f, 'be', 32).copy(publicKey, 1);
  } else {
    publicKey = Buffer$f.alloc(65);
    publicKey[0] = 0x04;
    x.toArrayLike(Buffer$f, 'be', 32).copy(publicKey, 1);
    y.toArrayLike(Buffer$f, 'be', 32).copy(publicKey, 33);
  }
...
var EC_PRIVKEY_EXPORT_DER_COMPRESSED = Buffer$f.from([
// begin
...
var EC_PRIVKEY_EXPORT_DER_UNCOMPRESSED = Buffer$f.from([
// begin
...
var privateKeyExport$1 = function (privateKey, publicKey, compressed) {
  var result = Buffer$f.from(compressed ? EC_PRIVKEY_EXPORT_DER_COMPRESSED : EC_PRIVKEY_EXPORT_DER_UNCOMPRESSED);
...
var signatureImportLax = function (signature) {
  var r = Buffer$f.alloc(32, 0);
  var s = Buffer$f.alloc(32, 0);
 ...
 * @return {boolean}
 */
var privateKeyExport$2 = function privateKeyExport(privateKey, compressed) {
  ...
  var publicKey = secp256k1Lib.privateKeyExport(privateKey, compressed);
...
var privateKeyImport$1 = function privateKeyImport(privateKey) {
...
var privateKeyNegate = function privateKeyNegate(privateKey) {
  return Buffer$f.from(secp256k1$1.privateKeyNegate(Uint8Array.from(privateKey)));
};
 ...
 * @return {Buffer}
 */
var privateKeyModInverse$1 = function privateKeyModInverse(privateKey) {
  if (privateKey.length !== 32) {
    throw new Error('private key length is invalid');
  }

  return Buffer$f.from(secp256k1Lib.privateKeyModInverse(Uint8Array.from(privateKey)));
};
...
var privateKeyTweakAdd = function privateKeyTweakAdd(privateKey, tweak) {
  return Buffer$f.from(secp256k1$1.privateKeyTweakAdd(Uint8Array.from(privateKey), tweak));
};
...
var privateKeyTweakMul = function privateKeyTweakMul(privateKey, tweak) {
  return Buffer$f.from(secp256k1$1.privateKeyTweakMul(Uint8Array.from(privateKey), Uint8Array.from(tweak)));
};
...
var publicKeyCreate = function publicKeyCreate(privateKey, compressed) {
  return Buffer$f.from(secp256k1$1.publicKeyCreate(Uint8Array.from(privateKey), compressed));
};
...
var publicKeyConvert = function publicKeyConvert(publicKey, compressed) {
  return Buffer$f.from(secp256k1$1.publicKeyConvert(Uint8Array.from(publicKey), compressed));
};
...
var publicKeyTweakAdd = function publicKeyTweakAdd(publicKey, tweak, compressed) {
  return Buffer$f.from(secp256k1$1.publicKeyTweakAdd(Uint8Array.from(publicKey), Uint8Array.from(tweak), compressed));
};
...
var publicKeyTweakMul = function publicKeyTweakMul(publicKey, tweak, compressed) {
  return Buffer$f.from(secp256k1$1.publicKeyTweakMul(Uint8Array.from(publicKey), Uint8Array.from(tweak), compressed));
};
...
var publicKeyCombine = function publicKeyCombine(publicKeys, compressed) {
  ...
  return Buffer$f.from(secp256k1$1.publicKeyCombine(keys, compressed));
};
...
var signatureNormalize = function signatureNormalize(signature) {
  return Buffer$f.from(secp256k1$1.signatureNormalize(Uint8Array.from(signature)));
};
...
var signatureExport = function signatureExport(signature) {
  return Buffer$f.from(secp256k1$1.signatureExport(Uint8Array.from(signature)));
};
 ...
 * @return {Buffer}
 */
var signatureImport$1 = function signatureImport(signature) {
  return Buffer$f.from(secp256k1$1.signatureImport(Uint8Array.from(signature)));
};
 ...
 * @return {Buffer}
 */
var signatureImportLax$1 = function signatureImportLax(signature) {
  ...
  return secp256k1Lib.signatureImport(sigObj);
};
...
var sign = function sign(message, privateKey, options) {
  ...
  if (options) {
    ...
    if (options.noncefn) {
      ...
      signOptions.noncefn = function (message, privateKey, algo, data, attempt) {
        var bufferAlgo = algo != null ? Buffer$f.from(algo) : null;
        var bufferData = data != null ? Buffer$f.from(data) : null;

        var buffer = Buffer$f.from('');

        if (options.noncefn) {
          buffer = options.noncefn(Buffer$f.from(message), Buffer$f.from(privateKey), bufferAlgo, bufferData, attempt);
        }
  ...
  return {
    signature: Buffer$f.from(sig.signature),
    recovery: sig.recid
  };
};
...
var recover = function recover(message, signature, recid, compressed) {
  ...
  return Buffer$f.from(secp256k1$1.ecdsaRecover(Uint8Array.from(signature), recid, Uint8Array.from(message), compressed));
};
...
var ecdh = function ecdh(publicKey, privateKey) {
  ...
  return Buffer$f.from(secp256k1$1.ecdh(Uint8Array.from(publicKey), Uint8Array.from(privateKey), {}));
};
 ...
 * @return {Buffer}
 */
var ecdhUnsafe$1 = function ecdhUnsafe(publicKey, privateKey, compressed) {
  ...
  return Buffer$f.from(secp256k1Lib.ecdhUnsafe(Uint8Array.from(publicKey), Uint8Array.from(privateKey), compressed));
};
...
function encode(input) {
    if (Array.isArray(input)) {
        ...
        var buf = Buffer$f.concat(output);
        return Buffer$f.concat([encodeLength(buf.length, 192), buf]);
    }
    else {
        var inputBuf = toBuffer(input);
        return inputBuf.length === 1 && inputBuf[0] < 128
            ? inputBuf
            : Buffer$f.concat([encodeLength(inputBuf.length, 128), inputBuf]);
    }
}
...
function encodeLength(len, offset) {
    if (len < 56) {
        return Buffer$f.from([len + offset]);
    }
    else {
        var hexLength = intToHex(len);
        var lLength = hexLength.length / 2;
        var firstByte = intToHex(offset + 55 + lLength);
        return Buffer$f.from(firstByte + hexLength, 'hex');
    }
}
function decode(input, stream) {
    if (stream === void 0) { stream = false; }
    if (!input || input.length === 0) {
        return Buffer$f.from([]);
    }
...
function intToBuffer(integer) {
    ...
    return Buffer$f.from(hex, 'hex');
}
/** Transform anything into a Buffer */
function toBuffer(v) {
    if (!Buffer$f.isBuffer(v)) {
        if (typeof v === 'string') {
            if (isHexPrefixed(v)) {
                return Buffer$f.from(padToEven(stripHexPrefix(v)), 'hex');
            }
            else {
                return Buffer$f.from(v);
            }
        }
            ...
            return Buffer$f.from(v.toArray());
        }
        else {
...
var streamBrowser = polyfillNode_events.EventEmitter;
...
var Buffer = polyfillNode_buffer.Buffer;

var inspect$2 = require$$0$1.inspect;
...
function () {
  ...
  _createClass(BufferList, [{
    ...
    value: function value(_, options) {
      return inspect$2(this, _objectSpread({}, options, {
...
var Buffer$1 = polyfillNode_buffer.Buffer;
...
polyfillNode_events.EventEmitter;
...
var Buffer$3 = polyfillNode_buffer.Buffer;
...
if (require$$0$1 && require$$0$1.debuglog) {
  debug$1 = require$$0$1.debuglog('stream');
} else {
...
var Transform$4 = readableBrowser.Transform;
...
inherits_browser(HashBase, Transform$4);
...
var Buffer$6 = polyfillNode_buffer.Buffer;
...
var Transform$5 = polyfillNode_stream.Transform;
var StringDecoder$3 = polyfillNode_string_decoder.StringDecoder;
...
inherits_browser(CipherBase, Transform$5);
...
function intToBuffer(i) {
  ...
  return new Buffer$f(padToEven(hex.slice(2)), 'hex');
}
...
function getBinarySize(str) {
  ...
  return Buffer$f.byteLength(str, 'utf8');
}
 ...
 */
function toUtf8(hex) {
  var bufferValue = new Buffer$f(padToEven(src$1(hex).replace(/^0+|0+$/g, '')), 'hex');
...
function fromUtf8(stringValue) {
  var str = new Buffer$f(stringValue, 'utf8');
...
var keccak224 = keccak$2.keccak224,
    keccak384 = keccak$2.keccak384,
    k256 = keccak$2.keccak256,
    keccak512 = keccak$2.keccak512;
 ...
 * @var {Object}
 */
exports.secp256k1 = secp256k1Adapter;
...
exports.sha256 = function (a) {
  a = exports.toBuffer(a);
  return browser$1('sha256').update(a).digest();
};
...
exports.ripemd160 = function (a, padded) {
  a = exports.toBuffer(a);
  var hash = browser$1('rmd160').update(a).digest();
  if (padded === true) {
...
exports.isValidPrivate = function (privateKey) {
  return secp256k1Adapter.privateKeyVerify(privateKey);
};
...
exports.isValidPublic = function (publicKey, sanitize) {
  if (publicKey.length === 64) {
    ...
    return secp256k1Adapter.publicKeyVerify(Buffer.concat([Buffer.from([4]), publicKey]));
  }
  ...
  return secp256k1Adapter.publicKeyVerify(publicKey);
};
...
exports.pubToAddress = exports.publicToAddress = function (pubKey, sanitize) {
  ...
  if (sanitize && pubKey.length !== 64) {
    pubKey = secp256k1Adapter.publicKeyConvert(pubKey, false).slice(1);
  }
  polyfillNode_assert(pubKey.length === 64);
...
var privateToPublic = exports.privateToPublic = function (privateKey) {
  ...
  return secp256k1Adapter.publicKeyCreate(privateKey, false).slice(1);
};
...
exports.importPublic = function (publicKey) {
  ...
  if (publicKey.length !== 64) {
    publicKey = secp256k1Adapter.publicKeyConvert(publicKey, false).slice(1);
  }
  return publicKey;
};
...
exports.ecsign = function (msgHash, privateKey) {
  var sig = secp256k1Adapter.sign(msgHash, privateKey);
...
exports.ecrecover = function (msgHash, v, r, s) {
  ...
  var senderPubKey = secp256k1Adapter.recover(msgHash, signature, recovery);
  return secp256k1Adapter.publicKeyConvert(senderPubKey, false).slice(1);
};
  ...
  fields.forEach(function (field, i) {
    ...
    function setter(v) {
      ...
      if (field.allowLess && field.length) {
        ...
        polyfillNode_assert(field.length >= v.length, 'The field ' + field.name + ' must not have more ' + field.length + ' bytes');
      } else if (!(field.allowZero && v.length === 0) && field.length) {
        polyfillNode_assert(field.length === v.length, 'The field ' + field.name + ' must have byte length of ' + field.length);
      }
...
export { Readable as R, Writable as W, isObject as a, isString as b, isNullOrUndefined as c, dist$1 as d, isNull as e, elliptic_1 as f, browser$1 as g, dist as h, inherits$1 as i, polyfillNode_assert as j, keccak$2 as k, polyfillNode_util$1 as p, secp256k1$1 as s };
